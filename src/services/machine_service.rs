use crate::models::machine::{
    DeployMachineRequest, MachineResponse, MaintenanceRequest, RegisterMachineRequest,
...
impl MachineService {
    ...
    pub async fn deploy_machine(
        &self,
        req: DeployMachineRequest,
    ) -> Result<MachineResponse, std::io::Error> {
        info!("Service deploy_machine");
        match self.repository.lock() {
            Ok(repository) => match repository.deploy(req).await {
                Ok(_) => Ok(MachineResponse::DeploySuccess),
                Err(error) => Ok(MachineResponse::Error(format!(
    ...
    pub async fn register_machine(
        ...
        match self.repository.lock() {
            Ok(repository) => match repository.register().await {
    ...
    pub async fn take_machine_for_maintenance(
        ...
        match self.repository.lock() {
            Ok(repository) => match repository.take_for_maintenance(req).await {
                Ok(_) => Ok(MachineResponse::TakenForMaintenanceSuccess),
                Err(error) => Ok(MachineResponse::Error(format!(
                    "Failed to take machine for maintenance: {}",
                    error
                ))),
            },
            Err(error) => Ok(MachineResponse::Error(format!(
